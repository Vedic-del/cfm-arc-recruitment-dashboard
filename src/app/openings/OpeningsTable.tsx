...
function groupByDepartment(openings: OpeningWithCount[]): [string, OpeningWithCount[]][] {
  const groups = new Map<string, OpeningWithCount[]>();
  for (const o of openings) {
    const dept = (o.department ?? '').trim() || NO_DEPT;
    (groups.get(dept) ?? groups.set(dept, []).get(dept)!).push(o);
  }
    ...
    <>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {groupByDepartment(openings).map(([dept, rows]) => (
              <Fragment key={dept}>
                <tr className="bg-slate-100/70">
                  <td colSpan={6} className="px-3 py-2 text-xs font-bold uppercase tracking-wide text-forest-900">
                    {dept} <span className="font-medium text-slate">· {rows.length} role{rows.length === 1 ? '' : 's'}</span>
                  </td>
                </tr>
                {rows.map((o) => (
                  <tr
